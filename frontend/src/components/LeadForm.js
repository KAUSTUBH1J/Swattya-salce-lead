...
const step1Schema = z.object({
  ...
  billing_type: z.string().optional(), // will enforce required conditionally below
}).superRefine((data, ctx) => {
  // Enforce billing_type when tender_type is Tender or Pre-Tender
  if (data.tender_type === 'Tender' || data.tender_type === 'Pre-Tender') {
    if (!data.billing_type || data.billing_type.trim() === '') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['billing_type'],
        message: 'Billing type is required for Tender / Pre-Tender'
      });
    }
  }
});
...
export const LeadForm = () => {
  ...
  // Form setup for current step: choose schema
  ...
  // Dynamic resolver so validation uses current step's schema at validation time
  const dynamicResolver = async (values, context, options) => {
    const schema = getCurrentSchema();
    // zodResolver returns a function; call it
  ...
  // Load master data and optionally lead data (edit)
  useEffect(() => {
    loadMasterData();
    if (isEdit) {
      loadLeadData();
    } else {
      // Try load saved draft from localStorage
      const saved = localStorage.getItem('leadFormData');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          setFormData(prev => ({ ...prev, ...parsed }));
          reset({ ...formData, ...parsed });
        } catch (e) {
          // ignore
        }
      }
  ...
  // Keep form defaults in sync when formData changes (e.g., after loading lead)
  useEffect(() => {
    reset(formData);
  }, [formData, reset]);

  // Auto-save functionality (save watched values + formData)
  useEffect(() => {
  ...
  const loadLeadData = async () => {
    try {
      ...
      leadData.sub_tender_type_id = leadData.sub_tender_type_id ?? 'none';
      leadData.partner_id = leadData.partner_id ?? 'none';
      leadData.product_service_id = leadData.product_service_id ?? 'none';
      leadData.checklist_completed = !!leadData.checklist_completed;
      leadData.billing_type = leadData.billing_type ?? '';
  ...
  const handleNext = async (data) => {
    ...
    // If not last step, go to next step. If last step, submit.
  ...
  const handleFinalSubmit = async (finalData) => {
    try {
      ...
      const submitData = {
        ...finalData,
        // ensure final checklist flag comes from checklistItems state
      ...
      // reset checklist and uploaded files
      setChecklistItems([
        { id: 'project_feasibility', label: 'Project feasibility assessed', checked: false },
        { id: 'budget_approval', label: 'Budget approval obtained', checked: false },
        { id: 'stakeholder_alignment', label: 'Stakeholder alignment confirmed', checked: false },
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      for (const file of files) {
        ...
        const response = await axios.post(`${API}/leads/upload-proof`, fd, {
  ...
  // Sync checklist boolean into react-hook-form so step3 validation can run
  useEffect(() => {
    const completed = checklistItems.every(item => item.checked === true);
    setValue('checklist_completed', completed, { shouldValidate: true, shouldDirty: true });
  }, [checklistItems, setValue]);

  // Watch tender_type to conditionally show billing field
  const currentTenderType = watch('tender_type');
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select onValueChange={(val) => {
                field.onChange(val);
                // when tender type changes, if not Tender/Pre-Tender clear billing_type
                if (!(val === 'Tender' || val === 'Pre-Tender')) {
                  setValue('billing_type', '', { shouldValidate: true });
                }
              }} value={field.value}>
      ...
      {/* Billing Type: only visible when tender_type is Tender or Pre-Tender */}
      {(currentTenderType === 'Tender' || currentTenderType === 'Pre-Tender') && (
        <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(handleNext)} className="space-y-6">
            {/* Hidden controller for checklist completion */}
            <Controller
            ...
            <div className="flex justify-between">
              ...
              <Button
                ...
                className={currentStep === 3 && !checklistItems.every(item => item.checked) ? 'opacity-50 cursor-not-allowed' : ''}
              >
                {submitting ? (
                  'Processing...'
                ) : currentStep === 3 ? (
