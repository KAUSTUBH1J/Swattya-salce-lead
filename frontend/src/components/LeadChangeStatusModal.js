...
export const LeadChangeStatusModal = ({ 
  ...
  useEffect(() => {
    ...
    const fetchLead = async () => {
        try {
            ...
            // Support both shapes: { lead: {...} } and {...}
            const payload = response.data?.lead ? response.data.lead : response.data;
            console.log('Fetched lead payload:', payload);

            // Normalize keys if backend uses different names (example)
            const normalized = {
                ...payload,
                company_name: payload.company_name ?? payload.company?.name ?? payload.companyName ?? payload.company_name_display,
                contact_phone: payload.contact_phone ?? payload.phone ?? payload.contactPhone,
                contact_email: payload.contact_email ?? payload.email ?? payload.contactEmail,
            };
  ...
  const handleStatusChange = async () => {
    ...
    try {
        ...
        // Immediately notify parent so the table / KPIs reload.
        // Call onSuccess even if we keep the modal open for conversion.
        if (onSuccess) {
            try { onSuccess(updatedLead); } catch (e) { console.warn('onSuccess callback failed', e); }
        }
        ...
        if (selectedStatus === 'approved' && !converted) {
            ...
            // Keep modal open to allow immediate conversion — parent already refreshed.
        } else {
